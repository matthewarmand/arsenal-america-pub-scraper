...
class ArsenalAmericaPubsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if mod_date != self.expected_article_date:
            print(
                "Warning! Expected article date is "
                + self.expected_article_date
                + " but actual is "
                + mod_date
                + ". Spider may be out of date. "
    ...
    def process_pubs(self, pubs):
        ...
        if os.path.getsize(file_name) >= 40000000:
            print("Warning! File size may be too large for Google Maps import")
